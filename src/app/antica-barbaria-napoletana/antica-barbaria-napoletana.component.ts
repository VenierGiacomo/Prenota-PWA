...
@Component({
  selector: 'app-antica-barbaria-napoletana',
  templateUrl: './antica-barbaria-napoletana.component.html',
  styleUrls: ['./antica-barbaria-napoletana.component.scss']
})
export class AnticaBarbariaNapoletanaComponent implements OnInit {
  year
  month
  day
  just_entered= 0
 ...
 availableSpots1:any =[]
  ...
  max_spots=-1
  id =40
  name ="Antica Barberia Napoletana"
  last_selected_week
  appointments
  book_site= false
  ...
  constructor(private api: ApiService, private router: Router, private titleService: Title) {
    this.titleService.setTitle( "Antica Barberia Napoletana  - Taglio uomo, barba e molto altro. Prendi appuntamento online");
   }
   async ngOnInit() {
...
goBookSite(){
  // if(this.mobileCheck()){
    window.location.href = 'https://mobile.prenota.cc/business/18'
  // }
}
