...
drawAppointment(id, start, end, details, client_name, employee, service, day ,week, month, year,note,payed,notvisible? ){
  ...
  div.onclick = function(){
    ...
    self.store_client = appo.store_client
    if(appo.client!=1){
...
async displayWeek(set_week){
  ...
  if(this.OneView ||  this.display =='month'){
    this.OneView=false
    this.display ='week'
    this.getAppoitments()
  }
...
// }    
 async setRecuringBooking(scale_credit){
  ...
  if(this.recurring_quantity<0){
    ...
    this.closeModal()
  this.api.setRecuringBooking(this.updateAppointmentId,this.recurring_quantity, scale_credit).subscribe((data:any)=>  {
    console.log(data)
    if(data.days_booked=='Giorni'){
...
storeClient(client_name,  phone,  credit, note){
