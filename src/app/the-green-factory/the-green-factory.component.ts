...
@Component({
  selector: 'app-the-green-factory',
  templateUrl: './the-green-factory.component.html',
  styleUrls: ['./the-green-factory.component.scss']
})
export class TheGreenFactoryComponent implements OnInit {
  year 
  month 
  ...
  this.api.getEmployeesfromshop(31).subscribe(data=>{
    this.employees_list =data 
    },err=>{
      console.log(err)
    })
this.api.getEmployeeservices(123).subscribe(async data=>{
  ...
  getServices(){
    ...
    this.api.getStoreserviceNoLogin(123).subscribe(
      data=>{
        this.services =  data
  ...
  getAppointments(day){
    ...
    this.api.getStoreAppointments(week,123).subscribe(
      data=>{
    ...
    // this.total_service.name=''
      for(let serv_ind in this.service){
        ...
        if(serv_ind =='0'){
          ...
          this.availableSpots=[]
          this.api.getemployeeHoursNoLogin(123).subscribe(
            async data=>{
        ...
        setTimeout(async () => {
          ...
          for (let empl of this.employees_serivces){
            if( empl.service_id == this.service[serv_ind].id){
                ...
                for(let spot of this.availableSpots){
                  // console.log(spot)
                  let obj = this.openhours.find(obj => obj.time == this.service[0].duration+spot.start && obj.employee == empl.employee);
                  if(obj!=undefined){
    ...
    for (let  ind in this.app_to_book){
      if (this.selected_date!='Seleziona data' &&  this.displ_hour!='Seleziona ora' && this.service !=''){
        if(this.api.isvalidToken()){
          ...
          this.api.bookAppointmentNoOwner(start, end, this.today, this.month, this.year, client_name, this.user.phone,  this.service[ind].name, this.app_to_book[ind].employee, this.app_to_book[ind].service,31).subscribe(async data=>{
            if( ind == '0'){
              this.toast_text= "Prenotazione andata a buon fine"
              this.toastx="block"
              this.sendEmailConfirmation(this.user.email,this.user.first_name,this.user.last_name,this.today,this.months_names[this.month],this.year,this.times[this.app_to_book[ind].start],this.service[ind].name,"The Green Factory")
      ...
      this.api.register(this.first_name, this.last_name,  this.email, this.sex, this.phone, this.password).subscribe(
        data=>{
          ...
          for (let  ind in this.app_to_book){
                ...
                var end = start + this.app_to_book[ind].duration
                // console.log(start, end, this.today, this.month, this.year, client_name, this.user.phone,  this.total_service.name, this.selected_hour.employee, this.total_service.id,23)
                this.api.bookAppointmentNoOwner(start, end, this.today, this.month, this.year, client_name, this.phone,  this.service[ind].name, this.app_to_book[ind].employee, this.app_to_book[ind].service,31).subscribe( async data=>{
                  if( ind == '0'){
                    this.toast_text= "Prenotazione andata a buon fine"
                    this.toastx="block"
                    this.sendEmailConfirmation(this.email,this.first_name,this.last_name,this.today,this.months_names[this.month],this.year,this.times[this.app_to_book[ind].start],this.service[ind].name,"The Green Factory")
                await  Notiflix.Report.Success("L'appuntamento è stato prenotato", 'Controlla la tua email per ulteriori informazioni', 'OK');
              var ok_btn = document.getElementById('NXReportButton')
              ok_btn.addEventListener("click",()=>{this.router.navigateByUrl('i_miei_appuntamenti')},false) 
                    this.selected_date='Seleziona data'
