...
declare var Stripe;
@Component({
  selector: 'app-tennis-club-grignano',
  templateUrl: './tennis-club-grignano.component.html',
  styleUrls: ['./tennis-club-grignano.component.scss']
})
export class TennisClubGrignanoComponent implements OnInit {
  year 
  month 
  ...
  constructor(private api: ApiService, private router: Router, private titleService: Title) {
    this.titleService.setTitle( "Prenota: Tennis Club Grignano - Gioca a tennis a Trieste. Prendi appuntamento online");
...
ngOnInit(){
  ...
  this.api.getEmployeesfromshop(30).subscribe(data=>{
    this.employees_list =data 
    },err=>{
      console.log(err)
    })
this.api.getEmployeeservices(107).subscribe(async data=>{
  ...
  getServices(){
    ...
    this.api.getStoreserviceNoLogin(107).subscribe(
      data=>{
        this.services =  data
  ...
  getAppointments(day){
    ...
    this.api.getStoreAppointments(week,107).subscribe(
      data=>{
  ...
  calculateAvailability(date){
    ...
    this.availableSpots=[]
    this.api.getemployeeHoursNoLogin(107).subscribe(
      data=>{
        var days =  data
        var list = [];
        var app
    ...
    if(this.first_name_err == '' && this.last_name_err == '' &&  this.email_err == '' && this.phone_err == '' && this.password_err == ''){
      ...
      this.api.register(this.first_name, this.last_name,  this.email, this.sex, this.phone, this.password).subscribe(
        data=>{
          ...
          var start = this.rows.indexOf(this.times[this.selected_hour.start])
          var end = start + this.total_service.duration
          this.api.bookAppointmentNoOwner(start, end, this.today, this.month, this.year, client_name, this.phone,  this.total_service.name, this.selected_hour.employee, this.total_service.id, 30).subscribe(data=>{
          this.register_form='none'
          this.user.first_name=this.first_name
          this.user.last_name=this.last_name
          this.sendEmailConfirmation(this.email,this.first_name,this.last_name,this.today,this.months_names[this.month],this.year,this.times[this.selected_hour.start],this.total_service.name,"Tennis Club Grignano")
          this.toast_text= "Prenotazione andata a buon fine"
          this.toastx="block"
          Notiflix.Report.Success("L'appuntamento è stato prenotato", 'Controlla la tua email per ulteriori informazioni', 'OK');
          this.selected_date='Seleziona data'
...
async pay(){
    ...
    const stripe = await Stripe('pk_test_f3m2iNJqa6UdyuD9Ey8O7ZiH00eSjJ4lEt');
    ...
    const style = {
      ...
      invalid: {
        ...
        var card = await elements.create('card', {style: style});
