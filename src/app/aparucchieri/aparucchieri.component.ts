...
@Component({
  selector: 'app-aparucchieri',
  templateUrl: './aparucchieri.component.html',
  styleUrls: ['./aparucchieri.component.scss']
})
export class AparucchieriComponent implements OnInit {
  year 
  month 
...
ngOnInit(){
  ...
  this.api.getEmployeesfromshop(23).subscribe(data=>{
    this.employees_list =data 
    },err=>{
      console.log(err)
    })
this.api.getEmployeeservices(45).subscribe(async data=>{
  ...
  getServices(){
    ...
    this.api.getStoreserviceNoLogin(45).subscribe(
      data=>{
        this.services =  data
  ...
  getAppointments(day){
    ...
    this.api.getStoreAppointments(week,45).subscribe(
      data=>{
  ...
  calculateAvailability(date){
    ...
    this.availableSpots=[]
    this.api.getemployeeHoursNoLogin(45).subscribe(
      data=>{
        var days =  data
        var list = [];
        var app
    ...
    if(this.first_name_err == '' && this.last_name_err == '' &&  this.email_err == '' && this.phone_err == '' && this.password_err == ''){
      ...
      this.api.register(this.first_name, this.last_name,  this.email, this.sex, this.phone, this.password).subscribe(
        data=>{
          ...
          var start = this.rows.indexOf(this.times[this.selected_hour.start])
          var end = start + this.total_service.duration
          this.api.bookAppointmentNoOwner(start, end, this.today, this.month, this.year, client_name, this.phone,  this.total_service.name, this.selected_hour.employee, this.total_service.id, 23).subscribe(data=>{
          this.register_form='none'
