...
export class CmassaggiComponent implements OnInit {
  ...
  constructor(private api: ApiService, private router: Router, private titleService: Title) {
    this.titleService.setTitle( "Prenota: CMassaggi - Massaggi olistici e del benessere a Trieste. Prendi appuntamento online");
   }
ngOnInit(){
    var now = new Date()
  ...
  this.api.getEmployeesfromshop(27).subscribe(data=>{
    this.employees_list =data 
    },err=>{
      console.log(err)
    })
this.api.getEmployeeservices(86).subscribe(async data=>{
  ...
  getServices(){
    ...
    this.api.getStoreserviceNoLogin(86).subscribe(
      data=>{
        this.services =  data
  ...
  getAppointments(day){
    ...
    this.api.getStoreAppointments(week,86).subscribe(
      data=>{
  ...
  calculateAvailability(date){
    ...
    this.availableSpots=[]
    this.api.getemployeeHoursNoLogin(86).subscribe(
      data=>{
        var days =  data
        var list = [];
        var app
  ...
  book(){
    ...
    if (this.selected_date!='Seleziona data' &&  this.displ_hour!='Seleziona ora' && this.service !=''){
      if(this.api.isvalidToken()){
        ...
        this.api.bookAppointmentNoOwner(start, end, this.today, this.month, this.year, client_name, this.user.phone,  this.total_service.name, this.selected_hour.employee, this.total_service.id,27).subscribe(data=>{
          this.toast_text= "Prenotazione andata a buon fine"
          this.toastx="block"
          this.sendEmailConfirmation(this.user.email,this.user.first_name,this.user.last_name,this.today,this.months_names[this.month],this.year,this.times[this.selected_hour.start],this.total_service.name,"CMassaggi")
    ...
    if(this.first_name_err == '' && this.last_name_err == '' &&  this.email_err == '' && this.phone_err == '' && this.password_err == ''){
      ...
      this.api.register(this.first_name, this.last_name,  this.email, this.sex, this.phone, this.password).subscribe(
        data=>{
          ...
          var start = this.rows.indexOf(this.times[this.selected_hour.start])
          var end = start + this.total_service.duration
          this.api.bookAppointmentNoOwner(start, end, this.today, this.month, this.year, client_name, this.phone,  this.total_service.name, this.selected_hour.employee, this.total_service.id, 27).subscribe(data=>{
          this.register_form='none'
          this.user.first_name=this.first_name
          this.user.last_name=this.last_name
          this.sendEmailConfirmation(this.email,this.first_name,this.last_name,this.today,this.months_names[this.month],this.year,this.times[this.selected_hour.start],this.total_service.name,"CMassaggi")
          this.toast_text= "Prenotazione andata a buon fine"
          this.toastx="block"
          Notiflix.Report.Success("L'appuntamento è stato prenotato", 'Controlla la tua email per ulteriori informazioni', 'OK');
          this.selected_date='Seleziona data'
